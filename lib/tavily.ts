...
async function tavilySearch(topic: string, query: string): Promise<ResearchResult> {
  const apiKey = process.env.TAVILY_API_KEY;

  if (!apiKey) {
    return { topic, query, answer: "Tavily key missing.", results: [] };
  }
  ...
  try {
    ...
    if (!response.ok) {
      return { topic, query, answer: "Search failed.", results: [] };
    }
